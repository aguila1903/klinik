  Page.setTitle("DOGAL TEDAVILER KLINIGI");
...
// Termin
  ...
  function drop(){// wird beim öffnen des Upload-Fensters gestartet
      var dropZone = document.getElementById('drop_zone');
      dropZone.addEventListener('dragover', handleDragOver, false);
      dropZone.addEventListener('drop', uploadFile, false);
  }

// Programmiere ich später aus
//function click() {// wird beim öffnen des Upload-Fensters gestartet
//    var dropZone = document.getElementById('drop_zone');
...
//}
  ...
  function uploadFile(event)
  {
      ...
      /*  xhr.addEventListener('load', function (event) {  //Server Response
       console.log(event);
      ...
      for(var i = 0, f; f = files[i]; i++){
          ...
          xhr.open('POST', 'api/az_upload.php'); // Die Verbindung wird geöffnet
          var formdata = new FormData();    // Anlegen eines FormData Objekts zum Versenden unserer Datei
          document.getElementsByName('kundenAttach').values = kundenListe.getSelectedRecord().lfd_nr; // zweiter Parameter Monat wird definie
          var name = kundenListe.getSelectedRecord().name;
          var vorname = kundenListe.getSelectedRecord().vorname;

          formdata.append('datei', f);  // Anhängen der Datei an das Objekt
          formdata.append('lfd_nr', document.getElementsByName("kundenAttach").values);  // Anhängen der Datei an das Objekt
          formdata.append('name', name);  // Anhängen der Datei an das Objekt
          formdata.append('vorname', vorname);  // Anhängen der Datei an das Objekt

          xhr.upload.addEventListener("progress", progressHandler, false); // ist für den Übertragungsprozess verantwortlich u.a. für die Progressbar
          xhr.addEventListener("load", completeHandler, false); // Gibt die Antwort des Servers wieder bei abgeschlossener Übertragung
          xhr.addEventListener("error", errorHandler, false); //Zeigt die Fehler-Texte an z.B. bei Fehlern im PHP Skript
          xhr.send(formdata);    // Absenden des Requests
      }
      //isc.say('<ul>' + output.join('') + '</ul>');

      document.getElementById('uploadList').innerHTML = '<ol>' + output.join('') + '</ol>'; //Namen der hochgel. Dateien werden angezeigt
...
//    document.getElementById('loaded_n_total').innerHTML = "Uploaded " + event.loaded + " bytes of " + event.total;
      ...
      document.getElementById('status').innerHTML = Math.round(percent) + "% yüklendi";
  }
...
// ICONS
  ...
  isc.TreeGrid.create({
      ...
      leafClick: function(_viewer, _node, _recordNum){
          if(_node.Id == "1010"){
              takvim.hide();
              VLayoutHastalar.show();
              VLayoutTedaviler.hide();
              VLayoutUser.hide();
              VLayoutRandevular.hide();
              VLayoutFaturalar.hide();
          } else if(_node.Id == "1011"){
              takvim.hide();
              VLayoutHastalar.hide();
              VLayoutTedaviler.show();
              VLayoutUser.hide();
              VLayoutRandevular.hide();
              VLayoutFaturalar.hide();
              ...
              takvim.show();
              //takvim.setContentsURL(calendar_);
              VLayoutHastalar.hide();
              VLayoutTedaviler.hide();
              VLayoutUser.hide();
              VLayoutRandevular.hide();
              ...
              VLayoutTedaviler.hide();
              VLayoutUser.hide();
              VLayoutRandevular.show();
              VLayoutFaturalar.hide();
          } else if(_node.Id == "1112"){
              if(admin == admin_){
                  CategoryTree.count++;
                  takvim.hide();
                  VLayoutFaturalar.show();
                  VLayoutHastalar.hide();
                  VLayoutTedaviler.hide();
                  VLayoutUser.hide();
                  VLayoutRandevular.hide();
...
//      title: '<text style="color:#3765A4; font-size:17px; font-family:Calibri; text-decoration:none;"><b>Çıkış</b></text>',
      title: '<text style="color:' + userFontColor + '; font-size:' + userFontSize + '; font-family:' + userFontFamily + '; text-decoration:none;"><center><b>Çıkış</b></center></text> ',
      width: 100,
      height: 40,
      padding: 5,
      align: "left",
...
//                });
  ...
  isc.DataSource.create({
      ...
      fields: [{name: "lfd_nr", type: "text", title: "#"},
          ...
          {name: "datum", type: "date", title: "Tarih"},
...
//-----AbrechnungSucheFelderDS-----------------------
  ...
  isc.DataSource.create({
      ...
      fields: [
          ...
          {name: "datum", type: "text", title: "Tarih"},
          {name: "name", type: "text", title: "Hasta ismi"},
  ...
  isc.DataSource.create({
      ID: "kundenListeAttachedDS",
      allowAdvancedCriteria: true,
      // serverType:"sql",
      dataFormat: "json",
      operationBindings: [
          {operationType: "fetch",
              dataURL: "api/ds/kundenListeAttachedDS.php"
          }
      ],
      titleField: "text",
      fields: [
          {name: "lfd_nr", type: "text", title: "", primaryKey: true},
          {name: "attach_id", type: "text", title: "i"},
          {name: "bezeichnung", title: "Ek", type: "text"},
          {name: "beleg_pfad", showTitle: false,
              type: "link",
              linkText: isc.Canvas.imgHTML("famfam/attach.png", 16, 16),
              linkURLPrefix: "api/attachments/"}
      ]
  });
 ...
 /*
  ...
  isc.ListGrid.create({
      ...
      fields: [
          ...
          {name: "gesamtpr_brutto", type: "text", width: "*", align: "right",
              recordSummaryFunction: "multiplier",
              summaryFunction: "sum",
              formatCellValue: function(value){
                  if(isc.isA.Number(value)){
          ...
          {name: "beleg_pfad", showTitle: false, width: 30, showIf: "true", align: "center"}
          ...
          buchungsListeDetails2.fetchRelatedData(record, buchungsListe_Randevu);
...
// Wird angezeigt wenn auf einenen vergangenen Termin geklickt wird
  isc.Window.create({
      ID: "wdTakvimRandevular",
      title: "",
      autoSize: false,
      autoCenter: true,
      showFooter: false,
  ...
  var agenda = function(datum_, view_){
      takvim.setContents("<div id='calendar'></div>");
      var _height = takvim.getInnerHeight();
      var _width = takvim.getInnerWidth();
...
//              isc.say(tag+"-"+mon+"-"+jahr);
                      ...
                      isc.say("Geçmiş Tarihe Randevu eklenemez");
                  }
...
/*
  ...
  isc.ListGrid.create({
      ...
      hilites: [
          ...
          var kundenListeAttached = isc.ListGrid.create({
              ID: "kundenListe_Attach",
              contextMenu: menuProdukteAttach,
              height: 120,
              width: 400,
              showHeader: false,
              cellheight: 22,
              leaveScrollbarGap: false,
              dataSource: kundenListeAttachedDS,
              canEdit: false,
              fields: [
                  {name: "lfd_nr", type: "text", showIf: "false", width: 80},
                  {name: "attach_id", showIf: "false", width: 50},
                  {name: "bezeichnung", type: "text", width: "*"},
                  {name: "beleg_pfad", width: 30, showIf: "true", align: "center"}
              ]
          });
          kundenListeAttached.fetchRelatedData(record, kundenListe);

          return kundenListeAttached;
      }, selectionChanged: function(record, state){
          if(state){
              tsbKundenDelete.setDisabled(false);
              tsbKundenEdit.setDisabled(false);
              tsbAddAbrechnungHasta.setDisabled(false);
          } else{
              tsbKundenDelete.setDisabled(true);
              tsbKundenEdit.setDisabled(true);
              tsbAddAbrechnungHasta.setDisabled(true);
          }
      ...
      pgbAddKundenFunction: function(){
          if(pgbAddKunden.percentDone < 100){
              ...
              if(!dfAddKunden.validate() && dfAddKunden.hasErrors()){
                  ...
                  isc.ask("Hasta eklendi.</br>Bir Hasta daha eklemek ister misiniz?", function(value){
                      if(value){
  ...
  isc.IButton.create({
      ID: "btnAddKundeTermin",
      type: "button",
      showDisabledIcon: false,
      icon: "famfam/calendar_add.png",
      disabled: false,
      name: "btnAddKundeTermin",
      title: "Randevu", width: 100, //Neuen Film anlegen
      click: function(){
          tsbAddAbrechnungHasta.action();
      }});
  ...
  isc.HLayout.create({
      ...
      members: [btnCloseKundeEdit, isc.LayoutSpacer.create({
              ...
              width: 20
          }), btnResetKundeEdit, isc.LayoutSpacer.create({
              width: 20
          }), btnAddKundeTermin]});
...
// define a KachelListen class (subclass of ListGrid)
  ...
  isc.ToolStripButton.create({
      ...
      action: function(){
          if(mwstListe.getSelection().length == 1){
              record = mwstListe.getSelectedRecord();
              dfEditMwst.editRecord(record);
              wdEditMwst.show();
              ...
              if(!dfAddProdukte.validate() && dfAddProdukte.hasErrors()){
                  ...
                  isc.ask("Tedavi başarıyla oluşturuldu. </ Br> Başka bir tedavi eklemek ister misiniz?", function(value){
                      if(value){
                          ...
                          dfAddProdukte.getField("prod_kz").focusInItem();
                          isc.Timer.setTimeout("btnSpeichernProduktNeu.findProdukt()", 300);
                          pgbAddProdukte.setTitle("");
                          pgbAddProdukte.setPercentDone(0);
      ...
      click: function(){
          ...
          RPCManager.send("", function(rpcResponse, data, rpcRequest){
              ...
              params: {
                  ...
                  passwort: hex_sha512(dfEditUser.getField("passwort").getValue() + userListe.getSelectedRecord().benutzer),
                  passwort2: hex_sha512(dfEditUser.getField("passwort2").getValue() + userListe.getSelectedRecord().benutzer),
                  orig_passwort: hex_sha512(dfEditUser.getField("orig_passwort").getValue() + userListe.getSelectedRecord().benutzer),
                  benutzer: userListe.getSelectedRecord().benutzer}
  ...
  isc.DynamicForm.create({
      ...
      fields: [{
          ...
          },
          {
              ...
              defaultValue: "<b>Randevuya  not ekleyin</b>",
              name: "Notiz",
              type: "section",
              colSpan: 4,
  ...
  isc.IButton.create({
      ...
      width: 100, //Neuen Film anlegen
      click: function(){
          var datum = dfAddAbrechnung.getField("datum").getValue();
          ...
          if(rundung(Ergebnis, 2) != aktErgebnis){
              ...
              RPCManager.send("", function(rpcResponse, data, rpcRequest){
                  ...
                  if(_data.response.status === 0){  // Durum 0 bedeutet Keine Hata
                      ...
                      //takvim.setContentsURL(calendar_);                      
                      agenda(convertCalDate(datum), _view);
              ...
              }, {// Eine neue Behandlung erstellen
      ...
      pgbAddAbrechnungFunction: function(){
          if(pgbAddAbrechnung.percentDone < 100){
              ...
              if(!dfAddAbrechnung.validate() && dfAddAbrechnung.hasErrors()){
                  ...
                  isc.ask("Randevu başarıyla oluşturuldu. </ br>Başka bir tedavi eklemek ister misiniz?", function(value){
                      if(value){
                          wdAddAbrechnung.hide();
                          wdAddPosition.show();
                      } else{
      ...
      fields: [
          {
              ...
              getPickListFilterCriteria: function(){
                  ...
                  var filter = {
                      count: dfAddPosition.verkaufCount, aktiv: "ja", beleg_nr: abrechnungsTree.getSelectedRecord().beleg_nr /*dfEditAbrechnung.getField("beleg_nr").getValue()*/}; 
                  return filter;
              }
  ...
  isc.IButton.create({
      ...
      click: function(){
          ...
          if(rundung(Ergebnis, 2) != aktErgebnis){
              ...
              }, {// Eine weitere Behandlung hinzufügen
                  actionURL: "api/addAbrechnung.php",
                  httpMethod: "POST",
                  contentType: "application/x-www-form-urlencoded",
                  useSimpleHttp: true,
  ...
  isc.ToolStripButton.create({
      ...
      title: "Kaydet",
      click: function(){
          var datum = dfEditAbrechnung.getField("datum").getValue();
  ...
  isc.HLayout.create({
      ...
      width: "100%",
      align: "center",
      members: [abrechnungsTree, takvim/*, VLayoutAbrechnungForm_Liste_Edit*/]});
...
//---------------ENDE Abrechnungssuche----------------------------------------
  ...
  isc.DynamicForm.create({
      ...
      fields: [{
              ...
              name: "freieSuche",
              title: "<b>Isim veya Belge No ara</b>",
              required: false,
              width: 200,
              colSpan: 1,
              type: "text",
  ...
  isc.ToolStripButton.create({
      ...
      count: 0,
      showDisabledIcon: false,
      icon: "web/48/excel.png",
      iconHeight: 32,
      iconWidth: 32,
      prompt: "Listeyi bir Excel Dosyasına aktarır.",
      hoverWidth: 100,
      hoverDelay: 700,
      action: function(){
  ...
  isc.ToolStripButton.create({
      ...
      action: function(){
          if(kundenListe.getSelection().length == 1){
              ...
              drop();
//            click();
              document.getElementsByName('kundenAttach').values = kundenListe.getSelectedRecord().lfd_nr;
              // ufArbeitsZeitenUpload.getField("monat").setValue(arbeitszeitenListe.getSelectedRecord().monat);
          } else{
              isc.say("Lütfen önce bir Hasta secin");
          }
      }
  });
  ...
  isc.ToolStripButton.create({
      ...
      action: function(){
          ...
          if(kundenListe_Attach.getSelection().length == 1){
              isc.ask("Gerçekten bu Eklemeyi kalıcı olarak silmek istiyor musunuz?", function(value){
                  if(value){
                      RPCManager.send("", function(rpcResponse, data, rpcRequest){
                          var _data = isc.JSON.decode(data); // Daten aus dem PHP (rpcResponse)
                          if(_data.response.status === 0){  // Durum 0 bedeutet Keine Hata
                              kundenListe_Attach.fetchData({count: ++tsbAttachDelete.count, lfd_nr: kundenListe_Attach.getSelectedRecord().lfd_nr});
                          } else{ // Wenn die Validierungen Hata aufweisen dann:
                          ...
                          actionURL: "api/deleteKundenAttach.php",
                          httpMethod: "POST",
                          contentType: "application/x-www-form-urlencoded",
                          useSimpleHttp: true,
                          params: {
                              attach_id: kundenListe_Attach.getSelectedRecord().attach_id,
                              bezeichnung: kundenListe_Attach.getSelectedRecord().bezeichnung
                          }
                      }); //Ende RPC 
                  }
              });
  ...
  /*
   ***************** Add Termin Button Hasta ***********************
   */
  isc.ToolStripButton.create({
      ID: "tsbAddAbrechnungHasta",
      title: "",
      showDisabledIcon: false,
      icon: "web/32/calendar_add.png",
      prompt: "Hastaya yeni bir randevu ekle",
      iconHeight: 32,
      iconWidth: 32,
      hoverWidth: 100,
      hoverDelay: 700,
      ...
      action: function(){
          if(kundenListe.getSelection().length == 1){
              if(wdEditKunden.isVisible() && wdEditKunden.isDrawn()){
                  wdEditKunden.hide();
              }
              ...
              pgbAddAbrechnung.setTitle("");
              pgbAddAbrechnung.setPercentDone(0);
              dfAddAbrechnung.getField("kunden_nr").fetchData();
//          isc.Timer.setTimeout("btnResetAbrechnungNeu.click()", 50);
              //Da Reset auch das Datum verändert und mit der Funktion setCalDate kollidiert, wird jedes Feld einzeld zurückgesetzt
              dfAddAbrechnung.getFields()[0].clearValue(); // beleg_nr
              dfAddAbrechnung.getFields()[1].setValue(kundenListe.getSelectedRecord().lfd_nr); // kunden_nr
              dfAddAbrechnung.getFields()[2].clearValue();
              dfAddAbrechnung.getFields()[3].clearValue();
              dfAddAbrechnung.getFields()[4].clearValue();
              dfAddAbrechnung.getFields()[8].clearValue();
              dfAddAbrechnung.getFields()[10].clearValue();
              dfAddAbrechnung.getFields()[11].clearValue();
              dfAddAbrechnung.getFields()[12].clearValue();
              dfAddAbrechnung.getFields()[13].clearValue();
              dfAddAbrechnung.getFields()[14].clearValue();
              dfAddAbrechnung.getFields()[15].clearValue();
              dfAddAbrechnung.getFields()[18].clearValue();
              dfAddAbrechnung.getField("kunden_nr").setDisabled(false);
              isc.Timer.setTimeout("preisFunction(dfAddAbrechnung)", 200);
              ...
              isc.Timer.setTimeout("getNextTermin(dfAddAbrechnung, dfAddAbrechnung.getField('datum').getValue(), 'J')", 150);
              //                            ;}
          } else{
              isc.say("Lütfen önce bir Hasta seçin");
          }
      }
  });
...
//          isc.Timer.setTimeout("btnResetAbrechnungNeu.click()", 50);
          //Da Reset auch das Datum verändert und mit der Funktion setCalDate kollidiert, wird jedes Feld einzeld zurückgesetzt
          dfAddAbrechnung.getFields()[0].clearValue();
          dfAddAbrechnung.getFields()[1].clearValue();
          dfAddAbrechnung.getFields()[2].clearValue();
          dfAddAbrechnung.getFields()[3].clearValue();
          dfAddAbrechnung.getFields()[4].clearValue();
          dfAddAbrechnung.getFields()[8].clearValue();
  ...
  isc.ToolStripButton.create({
      ...
      action: function(){
          ...
          laufendeNr = null;
          if(abrechnungsTree.getSelection().length == 1){
              isc.ask("<b> " + belegNr_ + " </b> nolu Randevuyu kalıcı olarak silmek istiyor musunuz?", function(value){
                  if(value){
              ...
              isc.say("Önce bir Randevu seçmelisiniz");
          }
  ...
  isc.ToolStripButton.create({
      ...
      iconHeight: 32,
      iconWidth: 32,
      prompt: "Bu isleme bir Fatura (PDF) ekleyin",
      hoverWidth: 200,
      hoverDelay: 700,
      action: function(){
          datum = buchungsListe.getSelectedRecord().datum;
          var Beleg_nr = buchungsListe.getSelectedRecord().beleg_nr;
          if(abrechnungsTree.getSelection().length == 1){
...
//                  if(value){
//                      var totalRows = buchungsListe.getTotalRows();
              RPCManager.send("", function(rpcResponse, data, rpcRequest){
                  var _data = isc.JSON.decode(data); // Daten aus dem PHP (rpcResponse)
                  if(_data.response.status === 0){  // Durum 0 bedeutet Keine Hata
                      isc.say(Beleg_nr + " nolu Randevuya başarıyla Fatura eklendi.");
                      //takvim.setContentsURL(calendar_);
//                              agenda(convertCalDate(datum), _view);
//                              if(totalRows == 3){
//                                  abrechnungsListeEdit.setData([]);
...
//                              }
//                              abrechnungsTree.invalidateCache();
                      isc.Timer.setTimeout("onRefresh('buchungsListe')", 100);
                      ...
                      RPCManager.send("", function(rpcResponse, data, rpcRequest){
                          ...
                          params: {
                              beleg_nr: Beleg_nr,
                              verkauf_an: buchungsListe.getSelectedRecord().verkauf_an,
                              datum: buchungsListe.getSelectedRecord().datum,
                              art: "buchen"}
                      }); //Ende RPC 
                  ...
                  useSimpleHttp: true,
                  params: {
                      beleg_nr: buchungsListe.getSelectedRecord().beleg_nr,
                      kunden_nr: buchungsListe.getSelectedRecord().verkauf_an,
                      kunden_name: buchungsListe.getSelectedRecord().name,
                      datum: buchungsListe.getSelectedRecord().datum
                  }
              }); //Ende RPC    
//                  } 
//              }, {title: " Randevu kaydetmek?"});
  ...
  isc.ToolStripButton.create({
      ...
      disabled: false,
      icon: "web/32/refresh.png",
      prompt: "Randevulari günceller ve Takvimi Ekrana göre ayarlar",
      hoverWidth: 100,
      hoverDelay: 700,
      action: function(){
  ...
  isc.Menu.create({
      ...
      data: [
          ...
          {title: "Dışa aktar...", icon: "icons/16/export1.png", submenu: [
                  {title: " EXCEL", icon: "web/16/excel.png", click: function(){
                          tsbKundenCSV.action();
                      }}]},
          {isSeparator: true},
          ...
          {isSeparator: true},
          {title: "Hastaya veri ekle", icon: "famfam/arrow_refresh.png", click: function(){
                  tsbPDF_Upload.action();
              }}
      ]
  });
...
//----------Anfang Menu Tedavi ------------------------------- 
  isc.Menu.create({
      ...
      data: [
          ...
          {title: "Tedavi düzenle", icon: "icons/32/doctor_edit.png", click: function(){
                  tsbProdukteEdit.action();
              }},
          {title: "Tedavi sil", icon: "icons/32/doctor_delete.png", click: function(){
  ...
  isc.Menu.create({
      ID: "menuProdukteAttach",
      autoDraw: false,
      showShadow: true,
      shadowDepth: 10,
      data: [
          {title: "Ekleme yap", icon: "famfam/attach.png", click: function(){
                  tsbPDF_Upload.action();
              }},
          {title: "Eklemeyi sil", icon: "famfam/delete.png", click: function(){
                  tsbAttachDelete.action();
              }}
      ]
  });
...
//----------Anfang Menu Abrechnung ------------------------------- 
  isc.Menu.create({
      ...
      showShadow: true,
      shadowDepth: 10,
      data: [/*{title: "Fatura ekle", icon: "famfam/accept.png", click: function(){
       tsbBookAbrechnung.action();
       }},*/
          {title: "Yeni Randevu oluştur", icon: "famfam/calendar_add.png", click: function(){
                  tsbAddAbrechnung.action();
              }},
          {title: "Randevuyu düzenle", icon: "famfam/calendar_edit.png", click: function(){
                  tsbAbrechnungGesamtEdit.action();
              }},
          {title: "Randevuyu sil", icon: "famfam/calendar_delete.png", click: function(){
  ...
  isc.Menu.create({
      ...
      data: [
          ...
          {title: "Tedavi listesini güncelle", icon: "famfam/arrow_refresh.png", click: function(){
                  tsbAbrechnungRefresh.count++;
                  onRefreshAbrechnung("abrechnungsListeEdit", dfEditAbrechnung.getField("beleg_nr").getValue(), tsbAbrechnungRefresh.count);
              }}
      ]
  });
...
//----------Anfang Menu Abrechnung ------------------------------- 
  isc.Menu.create({
      ...
      showShadow: true,
      shadowDepth: 10,
      data: [{title: "Fatura ekle", icon: "famfam/pdf.png", click: function(){
                  tsbBookAbrechnung.action();
              }}
      ]
  });
  ...
  isc.Window.create({
      ...
      LoadingStatusProgFoo2: function(){
          if(prBarLabelLoadingStatus.percentDone < 100){
              ...
              wdLoadBackupStatus.restoreFunction();
              wdLoadBackupStatus.hide();
...
//              SpieleSuchForm.clearValues();
          ...
          if(tabHauptFensterKlinik.getSelectedTabNumber() == 0){ // Günlük Randevular
              agenda(_Heute, _view);
              tsbAbrechnungRefresh.action();
          }
          ...
          if(tabHauptFensterKlinik.getSelectedTabNumber() == 3){ // Randevu Arsivleri
              dfBuchungenZeitraum.detailSuche(dfBuchungenZeitraum, "nein");
          }
          if(tabHauptFensterKlinik.getSelectedTabNumber() == 4){ // User            
              tsbUserRefresh.action();
          }
...
//                              CategoryTree.backupFunction();
  ...
  isc.IButton.create({
      ID: "btnUpdateWebClose",
      type: "button",
      showDisabledIcon: false,
      icon: "famfam/door_in.png",
      disabled: false,
      name: "btnUpdateWebClose",
      title: "Kapat", width: 100,
      click: function(){
  ...
  isc.HLayout.create({
      ID: "HLayoutBtnUpdateWebClose",
      height: 20,
      width: "100%",
      align: "center",
      margin: 5,
      members: [btnUpdateWebClose]});

  isc.VLayout.create({
      ID: "VLayoutUfUpdateWeb_BtnUpdateWebClose",
      height: "100%",
      width: "100%",
      align: "center",
      members: [ufUpdateWeb, HLayoutBtnUpdateWebClose]});


  isc.IButton.create({
      ID: "btnUpdateLocalClose",
      type: "button",
      showDisabledIcon: false,
      icon: "famfam/door_in.png",
      disabled: false,
      name: "btnUpdateLocalClose",
      title: "Kapat", width: 100,
      click: function(){
  ...
  isc.HLayout.create({
      ID: "HLayoutBtnUpdateLocalClose",
      height: 20,
      width: "100%",
      align: "center",
      margin: 5,
      members: [btnUpdateLocalClose]});

  isc.VLayout.create({
      ID: "VLayoutUfUpdateLocal_BtnUpdateLocalClose",
      height: "100%",
      width: "100%",
      align: "center",
      members: [ufUpdateLocal, HLayoutBtnUpdateLocalClose]});
  ...
  isc.TabSet.create({
      ...
      tabs: [
          {title: "Update Web",
              pane: VLayoutUfUpdateWeb_BtnUpdateWebClose},
          {title: "Update Local",
              pane: VLayoutUfUpdateLocal_BtnUpdateLocalClose}
      ],
      tabSelected: function(tabSet, tabNum, tabPane, ID, tab, name){
  ...
  isc.IButton.create({
      ID: "btnClosePDF_Upload",
      type: "button",
      disabled: false,
      icon: "famfam/door_in.png",
      name: "btnClosePDF_Upload",
      showDisabledIcon: false,
      title: "Kapat", width: 100,
      click: function(){
  ...
  isc.HLayout.create({
      ID: "HLayoutPDF_Upload",
      height: 30,
      width: "100%",
      align: "center",
      members: [btnClosePDF_Upload]});
  ...
  isc.HTMLPane.create({
      ...
      height: "100%",
      padding: 1,
      ID: "HtmlPaneDragNDrop",
      backgroundColor: "grey",
      styleName: "exampleTextBlock",
      contents: '<div id="main">' +
              '<div id="drop_zone">Verileri buraya cekin</div>' +
              '<form><input type="hidden" name="kundenAttach"></form>' +
  ...
  currentIcon = "famfam/picture_add.png";
  isc.Window.create({
      ID: "wdPDF_Upload",
      width: 402,
      height: 400,
      title: "Hastaya Veri Ekleme",
      //     autoSize: true,
      autoCenter: true,
      headerIconDefaults: {width: 16, height: 16, src: currentIcon},
      showFooter: false,
      ...
      canDragResize: true,
      align: "center",
      //  items: [ufArbeitsZeitenUpload, HLayoutPDF_Upload]});
      items: [HtmlPaneDragNDrop, HLayoutPDF_Upload]});
...
//      contents: '<text style="color:#3765A4; font-size:19px; font-family:Calibri; text-decoration:none;"><b>Ana veriler - Hastalar</b></text>'
      contents: '<text style="color:' + titleLableColor + '; font-size:' + titleLableFontSize + '; font-family:' + titleLableFontFamily + '; text-decoration:none;"><b>Ana veriler - Hastalar</b></text>'
  });
  ...
  isc.ToolStrip.create({
      ...
      members: [isc.LayoutSpacer.create({width: 10}),
          ...
          tsbAddAbrechnungHasta, isc.LayoutSpacer.create({width: 10}), "separator",
          tsbPDF_Upload, isc.LayoutSpacer.create({width: 10}), "separator",
...
//      contents: '<text style="color:#3765A4; font-size:19px; font-family:Calibri; text-decoration:none;"><b>Ana veriler - Tedaviler</b></text>'
      contents: '<text style="color:' + titleLableColor + '; font-size:' + titleLableFontSize + '; font-family:' + titleLableFontFamily + '; text-decoration:none;"><b>Ana veriler - Tedaviler</b></text>'
  });
...
//      contents: '<text style="color:#3765A4; font-size:19px; font-family:Calibri; text-decoration:none;"><b>Kullanıcılar</b></text>'
      contents: '<text style="color:' + titleLableColor + '; font-size:' + titleLableFontSize + '; font-family:' + titleLableFontFamily + '; text-decoration:none;"><b>Kullanıcılar</b></text>'
  });
...
//      contents: '<text style="color:#3765A4; font-size:19px; font-family:Calibri; text-decoration:none;"><b>Randevular</b></text>'
      contents: '<text style="color:' + titleLableColor + '; font-size:' + titleLableFontSize + '; font-family:' + titleLableFontFamily + '; text-decoration:none;"><b>Randevular</b></text>'
  });
  ...
  isc.Label.create({
      ...
      height: "100%",
      align: "center",
//      contents: '<text style="color:#3765A4; font-size:19px; font-family:Calibri; text-decoration:none;"><b>Randevu Arşivi</b></text>'
      contents: '<text style="color:' + titleLableColor + '; font-size:' + titleLableFontSize + '; font-family:' + titleLableFontFamily + '; text-decoration:none;"><b>Randevu Arşivi</b></text>'
  });
  ...
  isc.ToolStrip.create({
      ...
      width: "100%",
      height: 40,
      members: [tsbBookAbrechnung, "separator", isc.LayoutSpacer.create({width: 10}), isc.LayoutSpacer.create({width: 50}), lblBuchungen]});
  ...
  isc.HLayout.create({
      ...
      members: [
          VLayoutLogoutLabel/*, takvim, VLayoutHastalar, VLayoutTedaviler, VLayoutUser, VLayoutRandevular, VLayoutFaturalar , VLayoutKalender*/
      ]
  });
  isc.HLayout.create({
  ...
  isc.TabSet.create({
      ...
      tabs: [
          {title: "Günlük Randevular",
              ...
              pane: VLayoutRandevular},
          {title: "Hastalar",
              icon: "icons/32/patient.png",
              pane: VLayoutHastalar},
          {title: "Tedaviler",
              icon: "famfam/pill.png",
              pane: VLayoutTedaviler},
          {title: "Randevu Arşivi",
              icon: "famfam/page_white_acrobat.png",
              pane: VLayoutFaturalar},
      ...
      tabSelected: function(tabSet, tabNum, tabPane, ID, tab, name){
          ...
          if(tabHauptFensterKlinik.getSelectedTabNumber() == 3){//Randevu Arşivi
              CategoryTree.count++;
              CategoryTree.firstLoadFunction(dfBuchungenZeitraum, CategoryTree.count, Sene);
          }
...
//kundenListe_Attach.contextMenu = menuProdukteAttach;
  ...
  buchungsListe.contextMenu = menuBuchung;
//takvim.setContentsURL(calendar_);
