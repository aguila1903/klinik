...
$sqlQuery = "call addAusgabeEinzel("
...
if (!$rs) {
    ...
    $out['response']['errors'] = array('ausg_kz' => utf8_encode($dbSyb->ErrorMsg()));
...
If (isset($rs->fields['ergebnis'])) {
    if ($rs->fields['ergebnis'] != 1 && $rs->fields['ergebnis'] != 0) {
        $out['response']['status'] = -4;
        $out['response']['errors'] = array('ausg_kz' => "Es gab ein Problem beim Speichern in die Datenbank! </br>" . utf8_encode($dbSyb->ErrorMsg()));
    ...
    $out['response']['status'] = -4;
    $out['response']['errors'] = array('ausg_kz' => "Keine Ergebnis-Rückmeldung erhalten </br>" . utf8_encode($dbSyb->ErrorMsg()));
